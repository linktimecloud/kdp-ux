...
export default {
  ...
  computed: {
    iconPath () {
      ...
      return ICON_NAMES.includes(kind) ? `/img/k8sIcon/${kind}.png` : ''
    },
    kindLabel () {
  ...
  methods: {
    handleClickName () {
      if (this.isPod) {
        this.$emit('clickPod', this.properties)
      }
    }
  }
}
...
<template lang="pug">
.custom-node-wrapper.flex(:style="boxStyle")
  .icon-box
    AppIcon(v-if="properties.isRoot", :name="properties.appCatalog")
    img.w-full(v-else-if="iconPath" :src="iconPath")
