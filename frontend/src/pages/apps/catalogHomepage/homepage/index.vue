...
import SystemAppsList from '@/pages/apps/catalogHomepage/systemAppsList'
import AppsList from '@/pages/apps/catalogHomepage/appsList'
import InstallList from '@/pages/apps/catalogHomepage/installList'
import AppInstruction from '@/pages/apps/catalogHomepage/appInstruction'
  ...
  methods: {
    ...
    getComponentCls (item) {
      return {
        ...
        'h-100': item.name === 'appInstruction'
      }
    },
    refresh () {
...
.catalog-homepage(v-if="name")
  .catalog-homepage-header.px-3
    .d-flex.mb-2.align-items-center(v-if="isCatalogLevel")
      .img-box.d-flex.justify-content-between.align-items-center.mr-2
    ...
    .catalog-app-header(v-else)
      .d-block
        ...
        span.mr-2.ml-2 /
        span {{ sub }}
      .d-flex.align-items-center.mt-1.justify-content-between
        .d-flex.align-items-center
    ...
    el-tabs(v-model="activeTab")
      el-tab-pane(
        v-for="item in tabs",
        :key="`${item.name}${$store.state.lang}`",
  ...
  .catalog-container.bg-gray.p-3(:class="isCatalogLevel ? '' : 'app-container'")
    .catalog-component(
      ...
      .d-flex.justify-content-between.align-items-center(:class="{ 'mb-2': item.componentTitle }")
