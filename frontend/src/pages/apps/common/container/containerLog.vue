...
export default {
  ...
  methods: {
    ...
    getContainerLogs (line) {
      ...
      self.containerLogs = ''
      containerName && podName && appName && new Ajax({
        resource: {
          name: 'GET_APPLICATION_POD_CONTAINER_LOGS',
          sets: {
            appName, podName, containerName
          }
        },
        data: {
          tail_lines: lineNum
    ...
    downloadfile () {
      ...
      const fileName = `${podData.pod}-${containerName}-${strFull()}.log`
...
<template lang="pug">
.pod-container-log
  el-button(@click="handleOpen", type="text")
    slot
      span {{ $t('common.logs') }}
  el-drawer(
    custom-class="pod-container-log-dialog",
    ...
    .d-flex.mb-3
      .prepend-input.d-flex.mr-2(v-if="!defaultContainer")
