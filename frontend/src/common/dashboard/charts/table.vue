...
<template lang="pug">
.chart-table.h-100(v-loading="processing")
  el-table.border-left-0.border-right-0.h-100(
    v-if="list.length",
    :data="list",
    border,
    ...
    el-table-column(
      ...
      template(slot-scope="scope")
        ...
        ) {{ handleFormat(column, scope.row) | holder('-') }}
  .empty-holder-text.pt-4(v-else)
    img(v-if="parseInt(rawConfig.height) > 200", src="@/assets/img/empty_data.svg")
    p {{ $t('common.noData') }}
</template>
